...
| `moon run {{ package_name | kebab_case }}:migrate`                        | Apply pending Alembic database migrations                              |
| `moon run {{ package_name | kebab_case }}:migrate-create -- "name"`       | Create a new empty Alembic migration file with the specified name      |
| `moon run {{ package_name | kebab_case }}:migrate-autogenerate -- "name"` | Generate an Alembic migration by detecting model changes automatically |
| `moon run {{ package_name | kebab_case }}:migrate-down`                   | Rollback the most recent database migration                            |
| `moon run {{ package_name | kebab_case }}:migrate-reset`                  | Reset database by rolling back all migrations (useful for clean slate) |
| `moon run {{ package_name | kebab_case }}:seed`                           | Populate database with dummy data using the seeder script              |
| `moon run {{ package_name | kebab_case }}:check-in-dance`                 | Complete setup: sync, migrate, and seed in sequence                    |
